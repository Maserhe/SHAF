...
from data_utils import Fashion200kDataset, FashionIQDataset, ShoesDataset
import global_val
...
def extract_index_features(dataset: Union[Fashion200kDataset, FashionIQDataset, ShoesDataset], clip_model: CLIP) -> \
    ...
    # clip_model.vision_model.encoder.layers[6].register_forward_hook(hook_extract)

    # 中间输出
    # global_image_feature = torch.empty((0, 50, 768)).to(device, non_blocking=True)

    classic_val_loader = DataLoader(dataset=dataset, batch_size=32, num_workers=1,
                                    pin_memory=True, collate_fn=collate_fn)
    index_features = torch.empty((0, feature_dim)).to(device, non_blocking=True)
    index_names = []
    global_image_features = torch.empty((0, 12, 768)).to(device, non_blocking=True)

    if isinstance(dataset, Fashion200kDataset):
        print(f"extracting fashion200k {dataset.split} index features")
    ...
    for names, images in tqdm(classic_val_loader):
        images['pixel_values'] = images['pixel_values'].to(device)
        with torch.no_grad():
            batch_features = clip_model.get_image_features(**images)
            index_features = torch.vstack((index_features, batch_features))

            # 中间输出 global_image_feature
            global_image_features = torch.vstack((global_image_features, global_val.get_image_value()))
            global_val.clear_image()
            index_names.extend(names)
            
    return index_features, index_names, global_image_features
